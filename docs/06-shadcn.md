...
Load when: Use this for shadcn/ui, Radix primitives, component extension, variants, cn(), accessibility, and consistency.

## Staff-Level Intent

Treat shadcn/ui as a product quality system, not a cosmetic checklist.
...
- Inspect `components.json` before adding or changing shadcn components.
- Use existing shadcn components and local variants before creating new primitives.
- Extend components through variants and composition, not forks.
- Use `cn()` when available for predictable class merging.
- Keep Radix behavior intact: focus traps, roles, keyboard handling, portals.
- Do not strip accessibility props to simplify markup.
- Create variants only when the difference is reusable and named by intent.
- Keep destructive, secondary, outline, ghost, and link semantics consistent.
- Use data-state attributes for animations and state styling.
- Prefer local wrapper components for product-specific patterns.

## Implementation Heuristics

- Heuristic 1: Inspect `components.json` before adding or changing shadcn components. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Use existing shadcn components and local variants before creating new primitives. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Extend components through variants and composition, not forks. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Use `cn()` when available for predictable class merging. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Keep Radix behavior intact: focus traps, roles, keyboard handling, portals. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Do not strip accessibility props to simplify markup. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Create variants only when the difference is reusable and named by intent. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Keep destructive, secondary, outline, ghost, and link semantics consistent. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Use data-state attributes for animations and state styling. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Prefer local wrapper components for product-specific patterns. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Inspect `components.json` before adding or changing shadcn components. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Use existing shadcn components and local variants before creating new primitives. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Extend components through variants and composition, not forks. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Use `cn()` when available for predictable class merging. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Keep Radix behavior intact: focus traps, roles, keyboard handling, portals. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Do not strip accessibility props to simplify markup. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Create variants only when the difference is reusable and named by intent. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
