...
Load when: Use this for Recharts, KPI visualization, chart labels, legends, tooltips, dashboard data, and accessible charts.

## Staff-Level Intent

Treat charts as a product quality system, not a cosmetic checklist.
...
- Use charts to answer a decision, not to decorate a dashboard.
- Pick the simplest chart that reveals the relationship.
- Label axes, units, and time ranges clearly.
- Use legends only when direct labels are impractical.
- Tooltips must add value beyond the visible mark.
- Avoid too many series in one chart.
- Use color palettes that remain distinguishable in dark mode and color vision differences.
- Pair charts with summary metrics and interpretation when useful.
- Use loading and empty states that preserve chart dimensions.
- Do not animate data so heavily that values become hard to read.

## Implementation Heuristics

- Heuristic 1: Use charts to answer a decision, not to decorate a dashboard. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Pick the simplest chart that reveals the relationship. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Label axes, units, and time ranges clearly. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Use legends only when direct labels are impractical. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Tooltips must add value beyond the visible mark. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Avoid too many series in one chart. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Use color palettes that remain distinguishable in dark mode and color vision differences. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Pair charts with summary metrics and interpretation when useful. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Use loading and empty states that preserve chart dimensions. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Do not animate data so heavily that values become hard to read. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Use charts to answer a decision, not to decorate a dashboard. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Pick the simplest chart that reveals the relationship. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Label axes, units, and time ranges clearly. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Use legends only when direct labels are impractical. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Tooltips must add value beyond the visible mark. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Avoid too many series in one chart. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Use color palettes that remain distinguishable in dark mode and color vision differences. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
