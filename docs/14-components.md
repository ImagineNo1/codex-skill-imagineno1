...
Load when: Use this for reusable component APIs, states, variants, composition, prop design, and component boundaries.

## Staff-Level Intent

Treat components as a product quality system, not a cosmetic checklist.
...
- Name components by product role when domain-specific and by UI role when generic.
- Expose variants by intent, not by raw color or size trivia.
- Support loading, empty, error, disabled, hover, focus, and active states where relevant.
- Keep component APIs smaller than the implementation they hide.
- Prefer composition over boolean prop explosions.
- Use slots or children for flexible content regions.
- Do not leak implementation-specific class requirements to callers.
- Make destructive actions visually and semantically distinct.
- Document behavior through predictable props and examples in code.

## Implementation Heuristics

- Heuristic 1: A component should own one clear responsibility. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Name components by product role when domain-specific and by UI role when generic. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Expose variants by intent, not by raw color or size trivia. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Support loading, empty, error, disabled, hover, focus, and active states where relevant. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Keep component APIs smaller than the implementation they hide. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Prefer composition over boolean prop explosions. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Use slots or children for flexible content regions. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Do not leak implementation-specific class requirements to callers. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Make destructive actions visually and semantically distinct. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Document behavior through predictable props and examples in code. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: A component should own one clear responsibility. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Name components by product role when domain-specific and by UI role when generic. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Expose variants by intent, not by raw color or size trivia. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Support loading, empty, error, disabled, hover, focus, and active states where relevant. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Keep component APIs smaller than the implementation they hide. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Prefer composition over boolean prop explosions. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Use slots or children for flexible content regions. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
