...
Load when: Use this for folder organization, feature modules, shared logic, services, hooks, lib, types, and scalable frontend boundaries.

## Staff-Level Intent

Treat architecture as a product quality system, not a cosmetic checklist.
...
- Let routes describe navigation and features describe product capability.
- Keep shared UI primitives separate from domain components.
- Place service code near the boundary it serves.
- Use `lib` for stable utilities, not miscellaneous dumping.
- Use hooks for reusable client behavior, not as a hiding place for all logic.
- Keep types close to the domain that owns them unless shared broadly.
- Feature folders should contain cohesive UI, logic, tests, and helpers.
- Avoid circular dependencies between feature modules.
- Make data flow visible from route to component.
- Architect for the next likely change, not an imaginary platform rewrite.

## Implementation Heuristics

- Heuristic 1: Let routes describe navigation and features describe product capability. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Keep shared UI primitives separate from domain components. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Place service code near the boundary it serves. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Use `lib` for stable utilities, not miscellaneous dumping. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Use hooks for reusable client behavior, not as a hiding place for all logic. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Keep types close to the domain that owns them unless shared broadly. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Feature folders should contain cohesive UI, logic, tests, and helpers. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Avoid circular dependencies between feature modules. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Make data flow visible from route to component. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Architect for the next likely change, not an imaginary platform rewrite. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Let routes describe navigation and features describe product capability. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Keep shared UI primitives separate from domain components. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Place service code near the boundary it serves. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Use `lib` for stable utilities, not miscellaneous dumping. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Use hooks for reusable client behavior, not as a hiding place for all logic. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Keep types close to the domain that owns them unless shared broadly. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Feature folders should contain cohesive UI, logic, tests, and helpers. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
