...
Load when: Use this for App Router, routing, layouts, Server Components, Client Components, Suspense, streaming, caching, metadata, SEO, and rendering choices.

## Staff-Level Intent

Treat next.js as a product quality system, not a cosmetic checklist.
...
- Read local Next docs when project instructions warn that APIs differ.
- Use App Router conventions already present in the repo.
- Keep layout-level concerns in layouts, route-level content in pages, and reusable UI in components.
- Default to Server Components for data display and static composition.
- Use Client Components only for state, effects, browser APIs, or event handlers.
- Place Suspense boundaries around slow independent regions.
- Use streaming for meaningful progressive rendering, not as ceremony.
- Choose caching intentionally: static for stable content, dynamic for personalized or volatile content.
- Set metadata where pages need shareability, SEO, or correct titles.
- Use route groups to organize without changing URLs.

## Implementation Heuristics

- Heuristic 1: Read local Next docs when project instructions warn that APIs differ. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Use App Router conventions already present in the repo. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Keep layout-level concerns in layouts, route-level content in pages, and reusable UI in components. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Default to Server Components for data display and static composition. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Use Client Components only for state, effects, browser APIs, or event handlers. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Place Suspense boundaries around slow independent regions. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Use streaming for meaningful progressive rendering, not as ceremony. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Choose caching intentionally: static for stable content, dynamic for personalized or volatile content. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Set metadata where pages need shareability, SEO, or correct titles. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Use route groups to organize without changing URLs. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Read local Next docs when project instructions warn that APIs differ. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Use App Router conventions already present in the repo. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Keep layout-level concerns in layouts, route-level content in pages, and reusable UI in components. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Default to Server Components for data display and static composition. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Use Client Components only for state, effects, browser APIs, or event handlers. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Place Suspense boundaries around slow independent regions. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Use streaming for meaningful progressive rendering, not as ceremony. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
