...
Load when: Use this for safe UI refactors, decomposition, migration, design debt cleanup, and incremental modernization.

## Staff-Level Intent

Treat refactoring as a product quality system, not a cosmetic checklist.
...
- Capture the current behavior before changing structure.
- Extract components around stable responsibilities, not arbitrary line counts.
- Move styling into tokens before changing visual output.
- Replace duplicate UI through adapters when call sites differ.
- Keep old and new patterns from coexisting indefinitely.
- Validate after each meaningful step.
- Do not mix visual redesign and structural refactor unless the task requires both.
- Prefer reversible migrations.
- Leave the codebase with fewer concepts than you found.

## Implementation Heuristics

- Heuristic 1: Refactor in behavior-preserving slices. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Capture the current behavior before changing structure. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Extract components around stable responsibilities, not arbitrary line counts. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Move styling into tokens before changing visual output. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Replace duplicate UI through adapters when call sites differ. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Keep old and new patterns from coexisting indefinitely. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Validate after each meaningful step. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Do not mix visual redesign and structural refactor unless the task requires both. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Prefer reversible migrations. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Leave the codebase with fewer concepts than you found. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Refactor in behavior-preserving slices. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Capture the current behavior before changing structure. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Extract components around stable responsibilities, not arbitrary line counts. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Move styling into tokens before changing visual output. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Replace duplicate UI through adapters when call sites differ. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Keep old and new patterns from coexisting indefinitely. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Validate after each meaningful step. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
