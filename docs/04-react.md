...
Load when: Use this for composition, state, hooks, context, memoization, reusable architecture, and rendering optimization.

## Staff-Level Intent

Treat react as a product quality system, not a cosmetic checklist.
...
- Compose from small components with clear data ownership.
- Keep state as local as possible and as high as necessary.
- Derive values instead of duplicating state.
- Use context for cross-cutting state, not as a replacement for props everywhere.
- Memoize only when rendering cost or identity stability matters.
- Keep hooks deterministic and focused on one concern.
- Avoid effects for pure derivations.
- Prefer controlled APIs for reusable components that need orchestration.
- Use children and slots for layout flexibility.
- Make prop names describe product intent, not implementation trivia.

## Implementation Heuristics

- Heuristic 1: Compose from small components with clear data ownership. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Keep state as local as possible and as high as necessary. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Derive values instead of duplicating state. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Use context for cross-cutting state, not as a replacement for props everywhere. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Memoize only when rendering cost or identity stability matters. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Keep hooks deterministic and focused on one concern. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Avoid effects for pure derivations. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Prefer controlled APIs for reusable components that need orchestration. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Use children and slots for layout flexibility. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Make prop names describe product intent, not implementation trivia. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Compose from small components with clear data ownership. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Keep state as local as possible and as high as necessary. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Derive values instead of duplicating state. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Use context for cross-cutting state, not as a replacement for props everywhere. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Memoize only when rendering cost or identity stability matters. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Keep hooks deterministic and focused on one concern. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Avoid effects for pure derivations. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
