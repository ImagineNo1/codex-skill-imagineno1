...
Load when: Use this for data tables, TanStack Table, responsive strategy, sorting, filtering, row actions, empty states, and numeric alignment.

## Staff-Level Intent

Treat tables as a product quality system, not a cosmetic checklist.
...
- Tables are for comparison; cards are for individual summaries.
- Keep headers clear, sticky only when it improves scanning.
- Align text by reading direction and numbers by value comparison needs.
- Provide sorting and filtering only when users need them.
- Row actions must be discoverable without overwhelming each row.
- Use empty states that explain what happened and what to do next.
- Use skeletons that preserve table structure.
- On mobile, choose between priority columns, cards, or horizontal scroll deliberately.
- Use TanStack Table for complex state, not for simple static lists.
- Avoid hiding critical actions in inaccessible hover-only controls.

## Implementation Heuristics

- Heuristic 1: Tables are for comparison; cards are for individual summaries. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Keep headers clear, sticky only when it improves scanning. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Align text by reading direction and numbers by value comparison needs. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Provide sorting and filtering only when users need them. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Row actions must be discoverable without overwhelming each row. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Use empty states that explain what happened and what to do next. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Use skeletons that preserve table structure. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: On mobile, choose between priority columns, cards, or horizontal scroll deliberately. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Use TanStack Table for complex state, not for simple static lists. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Avoid hiding critical actions in inaccessible hover-only controls. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Tables are for comparison; cards are for individual summaries. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Keep headers clear, sticky only when it improves scanning. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Align text by reading direction and numbers by value comparison needs. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Provide sorting and filtering only when users need them. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Row actions must be discoverable without overwhelming each row. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Use empty states that explain what happened and what to do next. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Use skeletons that preserve table structure. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
