...
Load when: Use this for maintainability, naming, duplication, tests, linting, dead code, type safety, and production readiness.

## Staff-Level Intent

Treat code quality as a product quality system, not a cosmetic checklist.
...
- Make the code easier to change than it was before.
- Remove duplication when it creates risk, not when similarity is harmless.
- Use types to encode component contracts and data shapes.
- Keep naming boring, precise, and searchable.
- Do not leave dead variants, unused props, or stale CSS.
- Prefer explicit state machines for complex UI flows.
- Write tests where behavior is shared, fragile, or business-critical.
- Keep side effects isolated.
- Avoid broad refactors inside narrow UI fixes.
- Run the available validation commands and report failures honestly.

## Implementation Heuristics

- Heuristic 1: Make the code easier to change than it was before. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 2: Remove duplication when it creates risk, not when similarity is harmless. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 3: Use types to encode component contracts and data shapes. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 4: Keep naming boring, precise, and searchable. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 5: Do not leave dead variants, unused props, or stale CSS. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 6: Prefer explicit state machines for complex UI flows. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 7: Write tests where behavior is shared, fragile, or business-critical. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 8: Keep side effects isolated. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 9: Avoid broad refactors inside narrow UI fixes. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 10: Run the available validation commands and report failures honestly. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 11: Make the code easier to change than it was before. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 12: Remove duplication when it creates risk, not when similarity is harmless. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 13: Use types to encode component contracts and data shapes. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 14: Keep naming boring, precise, and searchable. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 15: Do not leave dead variants, unused props, or stale CSS. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 16: Prefer explicit state machines for complex UI flows. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
- Heuristic 17: Write tests where behavior is shared, fragile, or business-critical. Apply it through existing components, tokens, routes, and state boundaries before adding new abstractions.
