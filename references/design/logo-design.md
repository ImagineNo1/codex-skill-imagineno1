...
| Script | Purpose |
|--------|---------|
| `tools/logo/search.py` | Search styles, colors, industries; generate design briefs |
| `tools/logo/generate.py` | Generate logos with Gemini Nano Banana |
| `tools/logo/core.py` | BM25 search engine for logo data |
...
```bash
python3 tools/logo/search.py "tech startup modern" --design-brief -p "BrandName"
```
...
```bash
# Styles
python3 tools/logo/search.py "minimalist clean" --domain style

# Color palettes
python3 tools/logo/search.py "tech professional" --domain color

# Industry guidelines
python3 tools/logo/search.py "healthcare medical" --domain industry
```
...
```bash
python3 tools/logo/generate.py --brand "TechFlow" --style minimalist --industry tech
python3 tools/logo/generate.py --prompt "coffee shop vintage badge" --style vintage
```
...
1. Generate design brief â†’ `tools/logo/search.py --design-brief`
2. Generate logo variations â†’ `tools/logo/generate.py --brand --style --industry`
