...
| Script | Purpose |
|--------|---------|
| `tools/cip/search.py` | Search deliverables, styles, industries; generate CIP briefs |
| `tools/cip/generate.py` | Generate CIP mockups with Gemini (Flash/Pro) |
| `tools/cip/render-html.py` | Render HTML presentation from CIP mockups |
| `tools/cip/core.py` | BM25 search engine for CIP data |
...
```bash
python3 tools/cip/search.py "tech startup" --cip-brief -b "BrandName"
```
...
```bash
# Deliverables
python3 tools/cip/search.py "business card letterhead" --domain deliverable

# Design styles
python3 tools/cip/search.py "luxury premium elegant" --domain style

# Industry guidelines
python3 tools/cip/search.py "hospitality hotel" --domain industry

# Mockup contexts
python3 tools/cip/search.py "office reception" --domain mockup
```
...
python3 tools/cip/generate.py --brand "TopGroup" --logo /path/to/logo.png --deliverable "business card" --industry "consulting"

# Full CIP set with logo
python3 tools/cip/generate.py --brand "TopGroup" --logo /path/to/logo.png --industry "consulting" --set

# Pro model for 4K text rendering
python3 tools/cip/generate.py --brand "TopGroup" --logo logo.png --deliverable "business card" --model pro

# Custom deliverables with aspect ratio
python3 tools/cip/generate.py --brand "GreenLeaf" --logo logo.png --industry "organic food" --deliverables "letterhead,packaging,vehicle" --ratio 16:9

# Without logo (AI generates interpretation)
python3 tools/cip/generate.py --brand "TechFlow" --deliverable "business card" --no-logo-prompt
```

### Render HTML Presentation

```bash
python3 tools/cip/render-html.py --brand "TopGroup" --industry "consulting" --images /path/to/cip-output
python3 tools/cip/render-html.py --brand "TopGroup" --industry "consulting" --images ./topgroup-cip --output presentation.html
```
...
1. Generate CIP brief â†’ `tools/cip/search.py --cip-brief`
2. Generate mockups with logo â†’ `tools/cip/generate.py --brand --logo --industry --set`
3. Render HTML presentation â†’ `tools/cip/render-html.py --brand --industry --images`
